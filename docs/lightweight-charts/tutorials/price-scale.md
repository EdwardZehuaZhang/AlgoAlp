# Tutorial: Price Scale

*Source: tutorials\customization\price-scale.html*
...
It is recommend that you follow the guide from the [start](intro.md). However, if you are only interested in the content on this page then take a look at the [full code](price-scale.html#complete-code) at the bottom of page for context of how this code fits into a working example.
...
## Adjusting settings for the price scale[​](price-scale.html#adjusting-settings-for-the-price-scale "Direct link to Adjusting settings for the price scale")

We can get the current [IPriceScaleApi](../../docs/api/interfaces/IPriceScaleApi.md) instance for the chart by evoking the `priceScale()` method on the candlestick series reference.

Once again, we can use the `applyOptions()` method on this API instance to adjust it's options. You can add the following code to the example at any point after the `mainSeries` reference has been created, so let us place it at the end of the script. The options available are shown here: [PriceScaleOptions](../../docs/api/interfaces/PriceScaleOptions.md).
...
## Result[​](price-scale.html#result "Direct link to Result")
...
### Before[​](price-scale.html#before "Direct link to Before")

### After[​](price-scale.html#after "Direct link to After")

[View in a new window](../../4d50f2658b3f7a7f86b5facfaa59622f.md)

## Next steps[​](price-scale.html#next-steps "Direct link to Next steps")

In the next step, we will be adjusting the settings on the horizontal time scale.

## Download[​](price-scale.html#download "Direct link to Download")

You can download the HTML file for example at this stage [here](../../4d50f2658b3f7a7f86b5facfaa59622f.md) in case you've encountered a problem or would like to start the next step from this point.

## Complete code[​](price-scale.html#complete-code "Direct link to Complete code")
